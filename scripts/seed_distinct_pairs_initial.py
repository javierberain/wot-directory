#!/usr/bin/env python3
"""
seed_distinct_pairs_initial.py - the ORIGINAL one-time batch seed of the
distinct_pairs Check E suppression table.

This is a historical record. These 17 confirmed-distinct pairs were the initial
batch seeded into db/wot_book5.db (and they carry forward to later snapshots
because start_book.py copies the previous book's snapshot). They are already
applied to the live snapshots; this script is kept only so the original batch
remains reproducible. For new pairs, use scripts/seed_distinct_pairs.py, which
takes a single pair on the command line.
...
so re-running neither errors nor duplicates. A backup of the DB is taken first.
Pairs referencing a character_id no longer present (e.g. a character later
merged or deleted) are reported and skipped rather than crashing on a foreign
key failure.

Usage:
    python scripts/seed_distinct_pairs_initial.py                      # db/wot_book5.db
    python scripts/seed_distinct_pairs_initial.py --db db/wot_book5.db
"""
import argparse
import os
import shutil
...
def main():
    ap = argparse.ArgumentParser(
        description="Seed the ORIGINAL 17-pair batch into the distinct_pairs "
                    "Check E suppression table (historical record).")
